...
import AttackKnowledgePromotionReview from "./AttackKnowledgePromotionReview";
...
function CandidateCard({ candidate, onChanged }: { candidate: MemoryCandidate; onChanged: () => void }) {
  ...
  return (
    <article className="brain-candidate">
      ...
      {editing && <fieldset className="brain-candidate-editor"><legend>Edit before confirming</legend><label>Title<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Summary<textarea value={summary} onChange={(event) => setSummary(event.target.value)} /></label><label>Note body<textarea value={body} onChange={(event) => setBody(event.target.value)} /></label><div className="os-field-grid"><label>Sensitivity<TitaniumSelect value={sensitivity} onChange={(event) => setSensitivity(event.target.value as MemorySensitivity)}>{MEMORY_SENSITIVITIES.map((item) => <option key={item}>{item}</option>)}</TitaniumSelect></label><label>Scope<TitaniumSelect value={scopeKind} onChange={(event) => setScopeKind(event.target.value as MemoryScope["kind"])}><option value="global">Global</option><option value="engagement">Engagement</option><option value="mission">Mission</option></TitaniumSelect></label></div>{scopeKind !== "global" && <label>Engagement ID<input value={engagementId} onChange={(event) => setEngagementId(event.target.value)} required={scopeKind === "engagement"} /></label>}{scopeKind === "mission" && <label>Mission ID<input value={missionId} onChange={(event) => setMissionId(event.target.value)} required /></label>}</fieldset>}
...
export default function BrainInboxPage() {
  ...
  const candidateChanged = () => {
    candidates.refresh();
    setPromotionRevision((revision) => revision + 1);
  };
  return (
    <div className="os-page brain-page">
      ...
      <BrainNav />
      <AttackKnowledgePromotionReview candidateRevision={promotionRevision} />
      ...
      {pending.length > 0 && <div className="brain-candidate-list">{pending.map((candidate) => <CandidateCard key={candidate.id} candidate={candidate} onChanged={candidateChanged} />)}</div>}
