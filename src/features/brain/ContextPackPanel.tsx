...
export function ContextPackPanel({
  packId,
  retryControlId = "brain-context-pack-retry",
  retryLabel = "Retry context pack",
}: {
  packId: string;
  ...
  if (pack.error && !pack.data) return <ErrorPanel
    title="Context use is unavailable"
    error={pack.error}
    onRetry={pack.refresh}
    retryControlId={retryControlId}
  ...
  return (
    <section className="brain-context-pack" aria-labelledby={titleId}>
      ...
      <ul>{pack.data.items.map((item) => <li key={item.node.id} className={item.used ? "is-used" : "is-ignored"}><div><ButtonLink href={`/brain/nodes/${encodeURIComponent(item.node.id)}`} variant="quiet" aria-label={`Open memory node ${item.node.title}`}>{item.node.title}</ButtonLink><StatusPill status={item.used ? "used" : "ignored"} /></div><p>{item.relevanceReason}</p>{item.used && item.influenceSummary && <small>Influence: {item.influenceSummary}</small>}{!item.used && item.ignoredReason && <small>Ignored: {item.ignoredReason}</small>}<span>{scopeLabel(item.node.scope)} · {Math.round(item.node.confidence * 100)}% confidence</span></li>)}</ul>
    </section>
  );
}
