...
export default function BrainNodePage({ nodeId }: { nodeId: string }) {
  ...
  return (
    <div className="os-page brain-page brain-node-page">
      ...
      {detail.error && !detail.data && <ErrorPanel
        error={detail.error}
        onRetry={detail.refresh}
        retryControlId="brain-node-record-retry"
        retryLabel="Retry memory record"
      />}
      {detail.data && (() => {
        ...
        return <>
          ...
          <div className="brain-node-layout">
            ...
            <aside>
              ...
              {operation.message && <p className="brain-mutation-note" role="status">{operation.message}</p>}{operation.error && <ErrorPanel
                error={operation.error}
                onRetry={() => { setOperation({}); detail.refresh(); }}
                retryControlId="brain-node-mutation-reconcile"
                retryLabel="Refresh memory after failed change"
              />}
            </aside>
          </div>
