...
const source = {
  ...
  commanderProvider: [
    ...
    "server/guided-commander/LocalGuidedManualInterpreter.ts",
  ...
  hazardRuntime: ["server/routes/missionRuntimeV2Routes.ts"],
  hazardObservation: ["server/memory/OperationalHazardObservationRouter.ts"],
  attackKnowledge: ["server/migration/AttackKnowledgePromotionRouter.ts"],
  ...
  reusableExploitOnboarding: [
    "server/autonomous-runtime/ReusableExploitProcedureOnboardingRouter.ts",
  ],
  ...
  modelConfiguration: ["server/model-config/ModelConfigurationRouter.ts"],
  openRouterConnection: ["server/providers/openrouter/OpenRouterConnectionRouter.ts"],
} as const;
...
export const V2_MUTATION_AUTHORITY_INVENTORY = [
  ...
  protectedMutation({
    method: "POST",
    path: "/api/v2/brain/attack-knowledge/bundles/:fingerprint/preview",
    owningService: "AttackKnowledgePromotionService",
    implementationSources: source.attackKnowledge,
    idempotency: "not_required_read_like",
    optimisticConcurrency: "not_applicable",
    notes: "Read-only POST accepts an exact bounded evidence selection and computes a review hash without changing candidates, graph state, or evidence.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/brain/attack-knowledge/bundles/:fingerprint/promote",
    owningService: "AttackKnowledgePromotionService",
    implementationSources: source.attackKnowledge,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    notes: "An authenticated operator promotes only the exact reviewed hash and canonical provenance-bound evidence set inside one audited transaction.",
  }),
  ...
  globalMutation({
    method: "POST",
    path: "/api/v2/brain/vault/attack-knowledge-preset/activate",
    owningService: "ObsidianVaultBridge/AttackKnowledgeVaultPreset",
    implementationSources: source.brain,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Requires reusable-global-memory authority, explicit path permission, explicit activation acknowledgement, the exact current preset policy hash, a fresh write/read/rename/delete round-trip, and an attributable activation audit. It creates no note projection.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/brain/vault/attack-knowledge-preset/amend",
    owningService: "ObsidianVaultBridge/AttackKnowledgeVaultPolicyService",
    implementationSources: source.brain,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Requires reusable-global-memory authority, explicit filesystem permission, exact current and target policy hashes, the current connection version, and an explicit acknowledgement before broadening the same Vault to confirmed knowledge or the authenticated operator's consent-backed profile.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/brain/vault/:connectionId/disconnect",
    owningService: "VaultConnectionLifecycleService",
    implementationSources: source.brain,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Requires workspace-wide Second Brain authority, ti_scale control-plane ownership, explicit acknowledgement and reason, the exact current connection version, and either a healthy replacement or controlled-degraded-state acknowledgement. It changes only connection status and immutable audit state; Vault files and notes remain untouched.",
  }),
  ...
  globalMutation({
    method: "POST",
    path: "/api/v2/research/campaigns/:campaignId/setup",
    owningService: "ResearchLabRepository",
    implementationSources: source.research,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Only the named human campaign owner may approve the built-in charter and exact immutable local-bwrap benchmark setup. The route exposes no private holdout and grants no production execution or deployment authority.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/research/experiments/:experimentId/runs",
    owningService: "ExperimentRunner",
    implementationSources: source.research,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    notes: "Only the named human campaign owner may queue the exact snapshot-bound synthetic development scenario. Admission requires fresh HMAC receipts from the reviewed local-bwrap lab and worker boundaries.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/research/experiments/:experimentId/stages/:stage/runs",
    owningService: "ExperimentRunner",
    implementationSources: source.research,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    notes: "Only the named human campaign owner may queue the next ordered validation or private hidden-holdout stage. The caller supplies only a seed; ExperimentRunner resolves the single immutable snapshot-bound scenario locally, enforces the current lifecycle and budgets transactionally, and never exposes the private holdout identifier.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/research/experiments/:experimentId/runs/:runId/cancel",
    owningService: "ExperimentRunner",
    implementationSources: source.research,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    notes: "Only the named human campaign owner may cancel the exact represented run. Terminal cancellation is recorded only after the isolated process and disposable fixture report cleanup.",
  }),
  globalMutation({
    method: "POST",
    path: "/api/v2/research/experiments/:experimentId/promotion",
    owningService: "ResearchPromotionLifecycleRepository",
    implementationSources: source.research,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "A named human may invoke only the actions represented by the current immutable lifecycle. Benchmark advances require locally verified integrity and disclosure receipts; shadow has no live effect; canary is explicitly bounded; verification never deploys automatically; rollback appends a forward activation pointer.",
  }),
  globalMutation({
    method: "PUT",
    path: "/api/v2/model-preferences/:scopeType/:scopeId",
    owningService: "ModelConfigurationService",
    implementationSources: source.modelConfiguration,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Appends one versioned model preference at the reviewed scope. Active runs retain their immutable launch-time assignments.",
  }),
  globalMutation({
    method: "PUT",
    path: "/api/v2/provider-connections/openrouter",
    owningService: "OpenRouterConnectionService",
    implementationSources: source.openRouterConnection,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Stores one secret-bearing provider configuration behind the service-owned filesystem boundary. Responses, audit records, and browser storage never receive the credential.",
  }),
  globalMutation({
    method: "PUT",
    path: "/api/v2/provider-connections/openrouter/attestation",
    owningService: "OpenRouterConnectionService/OpenRouterDurableReadinessVerifier",
    implementationSources: source.openRouterConnection,
    idempotency: "required",
    optimisticConcurrency: "required",
    notes: "Attests only the exact already-loaded configuration version and publishes a secret-free provider/model readiness projection.",
  }),
  ...
  ...["explain-more", "show-next-step", "use-another-approach"].map((action): MutationAuthorityRecord => missionMutation({
  ...
  missionMutation({
    method: "POST", path: "/api/v2/guided/:missionId/commander/interpret-result",
    owningService: "GuidedCommanderService/LocalGuidedManualInterpreter",
    implementationSources: source.commanderInterpret,
    idempotency: "required", optimisticConcurrency: "represented_boundary",
    controlPlaneEnforcement: "lease_fenced",
    notes: "The provider-backed, local deterministic ingestion, and unavailable implementations all require exact represented mission/run scope and current server-held mutation authority before replay or any retained result mutation.",
  }),
  missionMutation({
    method: "POST",
    path: "/api/v2/missions/:missionId/runs/:runId/actions/:actionId/operational-hazard-reset",
    owningService: "OperationalHazardObservationService",
    implementationSources: source.hazardObservation,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    controlPlaneEnforcement: "ownership_fenced",
    notes: "Records only a completed typed local reset action inside the exact Ti-Scale mission/run after capability authorization; it cannot execute a reset.",
  }),
  missionMutation({
    method: "POST",
    path: "/api/v2/missions/:missionId/runs/:runId/operational-hazards/reset-minimum-observations",
    owningService: "OperationalHazardObservationService",
    implementationSources: source.hazardObservation,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    controlPlaneEnforcement: "ownership_fenced",
    notes: "Preserves an authenticated operator's aggregate lower bound separately from exact procedure attribution and never increases a reusable exact occurrence count.",
  }),
  ...
  missionMutation({
    method: "POST",
    path: "/api/v2/runs/:runId/operational-hazards/retry-authorizations",
    owningService: "MissionRuntimeEngine/OperationalHazardHealthGate",
    implementationSources: source.hazardRuntime,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    controlPlaneEnforcement: "lease_fenced",
    notes: "One exact retry authorization requires the canonical V2 run, a locally verified health assessment, the represented attack attempt, and current server-held run authority.",
  }),
  ...
  missionMutation({
    method: "POST",
    path: "/api/v2/missions/:missionId/intelligence/cves/:recordId/review",
    owningService: "CveApplicabilityService",
    implementationSources: source.cve,
    idempotency: "required",
    optimisticConcurrency: "required",
    controlPlaneEnforcement: "lease_fenced",
    notes: "CVE review requires the exact canonical run, record review version, update timestamp, evidence selection, and current trusted runtime lease before replay, then rechecks authority inside the atomic review/receipt transaction.",
  }),
  ...
  globalMutation({
    method: "POST",
    path: "/api/v2/missions/:missionId/script-artifacts/:scriptArtifactId/reusable-exploit-procedure",
    owningService: "ReusableExploitProcedureOnboardingService",
    implementationSources: source.reusableExploitOnboarding,
    idempotency: "required",
    optimisticConcurrency: "represented_boundary",
    notes: "Operator-only global-memory publication is fenced to an exact V2-owned mission/run, latest approved immutable ScriptArtifact hash, confirmed CVE updated_at, custody-verified version-evidence hash, typed request/assertion observer spec, and one explicitly connected Vault. It has no execution or mission-authority port.",
  }),
  ...
  missionMutation({
    method: "POST",
    path: "/api/v2/runs/:runId/plan-changes/:requestId/resolve-inflight",
    owningService: "PlanChangeService",
    implementationSources: source.plan,
    idempotency: "required",
    optimisticConcurrency: "required",
    controlPlaneEnforcement: "lease_fenced",
    notes: "The boundary fences dispatch only for the exact request, run, plan, affected actions, and represented resolution mode after validating current versions and trusted runtime lease authority.",
  }),
  missionMutation({
    method: "POST",
    path: "/api/v2/runs/:runId/plan-changes/:requestId/inflight-resolution/finalize",
    owningService: "PlanChangeService",
    implementationSources: source.plan,
    idempotency: "required",
    optimisticConcurrency: "required",
    controlPlaneEnforcement: "lease_fenced",
    notes: "Finalization requires the exact current resolution version and trusted runtime lease before reconciling terminal work and preparing a fresh immutable review.",
  }),
  ...
  missionMutation({
    method: "POST", path: "/api/v2/reports/runs/:runId/generate",
    owningService: "CanonicalMissionReportService", implementationSources: source.operations,
    idempotency: "required", optimisticConcurrency: "represented_boundary",
    controlPlaneEnforcement: "read_only_import_compatible",
    notes: "Report generation writes only immutable, content-addressed V2 projection artifacts. It rechecks authorized mission/run scope, binds one report version to one canonical source hash, and never transfers or mutates source-run control authority, so imported legacy runs remain read-only.",
  }),
