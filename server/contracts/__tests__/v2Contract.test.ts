      ...
      const dynamicPrefix = file.endsWith("OperationalTruthRouter.ts")
        ...
        : file.endsWith("RunIntelligenceRouter.ts")
          ...
          : file.endsWith("CveApplicabilityRouter.ts")
            ? "/api/v2"
          : file.endsWith("MissionScopedNvdDetailRouter.ts")
            ? "/api/v2"
          : file.endsWith("PageCaptureRouter.ts")
            ? "/api/v2"
          : file.endsWith("ScriptArtifactRouter.ts")
            ? "/api/v2"
          : file.endsWith("ReusableExploitProcedureOnboardingRouter.ts")
            ? "/api/v2"
          : file.endsWith("ModelConfigurationRouter.ts")
            ? "/api/v2"
...
function mountedImplementationRouteIds(): string[] {
  ...
  return routeIds([
    ...
    resolve(serverRoot, "system-capabilities/CapabilitySelfTestRouter.ts"),
    ...
    resolve(serverRoot, "memory/OperationalHazardObservationRouter.ts"),
    resolve(serverRoot, "migration/AttackKnowledgePromotionRouter.ts"),
    resolve(serverRoot, "model-config/ModelConfigurationRouter.ts"),
    resolve(serverRoot, "providers/openrouter/OpenRouterConnectionRouter.ts"),
    ...
    resolve(serverRoot, "cve-intelligence/MissionScopedNvdDetailRouter.ts"),
    resolve(serverRoot, "page-captures/PageCaptureRouter.ts"),
    resolve(serverRoot, "script-artifacts/ScriptArtifactRouter.ts"),
    resolve(serverRoot, "autonomous-runtime/ReusableExploitProcedureOnboardingRouter.ts"),
...
describe("Ti-Scale contract", () => {
  ...
  test("has stable unique operations and idempotency on harmful mutations", () => {
    ...
    const unsafeWithoutKey = TI_SCALE_ENDPOINTS.filter((endpoint) =>
      ...
      && endpoint.path !== "/api/v2/brain/attack-knowledge/bundles/:fingerprint/preview"
  ...
  test("emits a valid OpenAPI 3.1 document with shared errors", () => {
    ...
    expect(document.paths["/api/v2/system/capability-self-tests"]?.get).toBeDefined();
    expect(document.paths["/api/v2/provider-connections/openrouter"]?.get)
      .toMatchObject({ "x-idempotency-required": false });
    expect(document.paths["/api/v2/provider-connections/openrouter"]?.put)
      .toMatchObject({ "x-idempotency-required": true });
    expect(document.paths["/api/v2/provider-connections/openrouter/attestation"]?.put)
      .toMatchObject({ "x-idempotency-required": true });
    ...
    expect(document.paths["/api/v2/brain/attack-knowledge/bundles"]?.get).toBeDefined();
    expect(document.paths["/api/v2/brain/attack-knowledge/bundles/{fingerprint}/evidence"]?.get).toBeDefined();
    expect(document.paths["/api/v2/brain/attack-knowledge/bundles/{fingerprint}/preview"]?.post)
      .toMatchObject({ "x-idempotency-required": false });
    expect(document.paths["/api/v2/brain/attack-knowledge/bundles/{fingerprint}/promote"]?.post)
      .toMatchObject({ "x-idempotency-required": true });
    ...
    expect(document.paths["/api/v2/missions/{missionId}/intelligence/cves/{recordId}/review"]?.post)
      .toMatchObject({
        "x-idempotency-required": true,
      });
    expect(document.paths["/api/v2/missions/{missionId}/runs/{runId}/steps/{stepId}/intelligence/cves/{recordId}/nvd-detail"]?.get)
      .toMatchObject({
    ...
    expect(document.paths[
      "/api/v2/missions/{missionId}/script-artifacts/{scriptArtifactId}/reusable-exploit-procedure"
    ]?.post).toMatchObject({ "x-idempotency-required": true });
    ...
    expect(document.paths["/api/v2/brain/preferences"]?.get).toBeDefined();
